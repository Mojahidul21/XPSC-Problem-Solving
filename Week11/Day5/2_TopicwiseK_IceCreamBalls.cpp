...
#define tourist ios::sync_with_stdio(0);cin.tie(0);
#define pss <<' '
#define pnl cout<<'\n';
#define tnl if(t) pnl
...
int main(){
    ...
    while(t--){
        ...
        auto ok = [&](lli mid){
            return mid * (mid - 1) / 2 + mid >= n;
        };

        lli l = 0, r = min<lli>(2e9, 2 * n), m, ans;
        while (l <= r) {
            m = (l + r) / 2;
            if (ok(m)) ans = m, r = m - 1;
            else l = m + 1;
        ...
        lli y = n - ans * (ans - 1) / 2;
        cout << ((ans + 1) * ans / 2 <= n ? min(ans + y, ans + 1 + n - (ans + 1) * ans / 2) : ans + y);
        tnl
    }
