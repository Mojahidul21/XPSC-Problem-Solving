...
int main(){
    ...
    int t;cin>>t;
    while(t--){
        int n;cin>>n;set<string>s;map<string,int>m;
        for(int i=1;i<=n;++i){string str;cin>>str;s.insert(str);m[str]++;};
        lli ans=0;auto i=s.begin(),j=s.begin(),nn=s.end();--nn;
        for(;i!=nn;++i){
            auto tmp=i;tmp++;
            for(j=tmp;j!=s.end();++j){
                string ii=*i,jj=*j;int diffCount=0;
                if(ii[0]!=jj[0])diffCount++;
                if(ii[1]!=jj[1])diffCount++;
                if(diffCount==1)ans+=1ll*m[ii]*m[jj];
            }
        }
        cout<<ans;
        tnl
