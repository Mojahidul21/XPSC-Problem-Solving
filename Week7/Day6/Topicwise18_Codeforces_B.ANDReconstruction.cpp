...
int main(){
    ...
    int t;cin>>t;
	while(t--){
        int n,flag=1;cin>>n;vector<int>a(n),b(n-1);
        for(int i=0;i<n-1;++i){cin>>b[i];a[i]|=b[i],a[i+1]|=b[i];}
        for(int i=0;i<n-1;++i)if((a[i]&a[i+1])!=b[i]){flag=0;break;}
        if(!flag){cout<<-1;tnl continue;}
        for(int i=0;i<=n-1;++i)cout<<a[i]pss;
        tnl
    }
